...
BOT_TOKEN = os.getenv("BOT_TOKEN", "7888086912:AAHj22J_7oGMk61WMiZcdw3InNsD3botpaA")
CHANNEL_USERNAME = "@vddownloaderuzbot"
ADMIN_ID = 5718626045
DOWNLOAD_DIR = "downloads"
WEBHOOK_URL = "https://vddownloader-bot-2.onrender.com/"  # Render domening
...
def handle_link(message):
    ...
    if not re.match(r"^https?://", url):
        bot.send_message(
            user_id,
            "⚠️ Bu to‘g‘ri link emas. Faqat https:// bilan boshlanadigan link yuboring.",
        )
        return
    ...
    try:
        ...
        subprocess.run(
            ["yt-dlp", "-f", "bestaudio", "-x", "--audio-format", "mp3", "-o", audio_output, url],
            check=True,
        )
        ...
        if video_files:
            with open(video_files[0], "rb") as vid:
                ...
                markup.add(
                    telebot.types.InlineKeyboardButton(
                        "🎵 Musiqani yuklab olish", callback_data="get_audio"
                    )
                )
        ...
        bot.edit_message_text(
            "❌ Yuklab bo‘lmadi. Linkni tekshirib qayta yuboring.", user_id, wait_msg.message_id
        )
    finally:
...
# === Webhook setup when app starts (instead of @app.before_first_request)
